...
class VirethIntroContractTest(unittest.TestCase):
    def test_intro_contract(self) -> None:
        ...
        expected_dates = tuple(date for date, _, _ in UPDATE_HISTORY)
        self.assertEqual(expected_dates, tuple(parser.update_dates))
        self.assertEqual(
            tuple((date, date) for date in expected_dates),
            tuple(parser.update_records),
        )
