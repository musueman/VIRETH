...
형식=`![](https://vireth-svg.musueman.workers.dev/scene?region={권역명}&place={세부장소명})`.
...
형식=`![](https://vireth-svg.musueman.workers.dev/map?region={권역명}&place={세부장소명})`.
...
[검수]
응답 첫 줄이 `/scene?region=...&place=...`가 아니면 실패.
...
~~~md
R=현재 권역/국가명. P=현재 세부장소명. 첫 줄은 `![](https://vireth-svg.musueman.workers.dev/scene?region=R&place=P)`. 상태창 코드블록 바로 아래는 `![](https://vireth-svg.musueman.workers.dev/map?region=R&place=P)`.
